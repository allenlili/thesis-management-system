	  ...
	  <caption class="caption alert-danger text-muted"><span class="icon-hand-right"></span>只有一次提交机会！</caption>
	    <thead>
	      <tr>
	        ...
	        <th class="text-center">评阅分数</th>
	        <th class="text-center">是否同意答辩</th>
	        <th class="text-center">确认时间</th>
	        <c:if test="${teacher.forwardViewerGradeStatus == 1 }">
	        <th class="text-center">操&nbsp;&nbsp;作</th>
	        </c:if>
	        <c:if test="${teacher.forwardViewerGradeStatus == 3 }">
	    ...
	    <tbody>
	      <c:forEach items="${students}" var="o">
	        <tr>
	          ...
	          <td class="text-center">${o.viewerAdvice.viewerGrade }</td>
	          
	          <c:if test="${o.viewerAdvice == null}">
	            <td class="text-center"></td>
	          </c:if>	          
	          <c:if test="${o.viewerAdvice.agreeOrNot == 0}">
	            <td class="text-center">否</td>
	          </c:if>
	          <c:if test="${o.viewerAdvice.agreeOrNot == 1 }">
	            <td class="text-center">是</td>
	          </c:if>
	          
	          <td class="text-center">${o.viewerAdvice.startDate }</td>
	          
	          <c:if test="${o.viewerAdvice == null && o.lastThesisScript != null}">
	          <td class="text-center">
	            <a href="${pageContext.request.contextPath}/main/viewerAdvice!toAdd.action?studentUuid=${o.uuid}">创建</a>
	          </td>
	          </c:if>
	          <c:if test="${o.viewerAdvice == null && o.lastThesisScript == null}">
	          ...
	          <c:if test="${o.viewerAdvice.status == 1 }">
	          <td class="text-center">
	            <a href="${pageContext.request.contextPath}/main/viewerAdvice!toUpdate.action?id=${o.viewerAdvice.id}">修改</a>
	          </td>
	          </c:if>
	          
	          <c:if test="${o.viewerAdvice.status == 3 }">
	          <td class="text-center">
<%-- 	            <a href="${pageContext.request.contextPath}/main/viewerAdvice!watch.action?id=${o.viewerAdvice.id}">查看</a> --%>
	            <a href="#" class="btn btn-mini" rel="popover" data-original-title="指导意见" data-content="${o.viewerAdvice.opinion}" data-placement="top" data-trigger="hover"><span class="glyphicon glyphicon-info-sign"></span></a>
	          </td>
	          </c:if>
    ...
    <script type="text/javascript">
    	function startForward(){
    		...
    		if(answer == true){
    			url='${pageContext.request.contextPath}/main/viewerAdvice!forwardViewerAdvice.action';
    			$.post(url);
