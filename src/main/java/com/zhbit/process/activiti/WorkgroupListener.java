...
@Component
public class WorkgroupListener implements TaskListener {
	...
	public void notify(DelegateTask task) {
		...
		if(SystemConstant.STUDENT.equals(requestUser.getUserType())){
			users = activitiDao.findWorkgroupForUserByStudentUser(requestUser);
		}else{
			users = activitiDao.findWorkgroupForUserByTeacherUser(requestUser);
		}
		for (User u : users) {
